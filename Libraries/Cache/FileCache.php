...
define ('LEDU_ALA_CACHE_DIR',CACHE_PATH);
...
class FileCache extends Cache
{
  ...
  */
  ...
  {
  	//if(null == $cacheDir)
		$cacheDir = LEDU_ALA_CACHE_DIR ;//Config::get('ala_cache_dir');
	/*else
		$cacheDir = Config::get('ala_cache_dir')."/$cacheDir";
*/
  ...
  public function initialize($options = array())
  {
    ...
    {
		//no need logging in ledu frame work 
      /*if (!in_array($key, $availableOptions) && Config::get('logging_enabled'))
      {
			$logger = new Logger();
			$logger->error(sprintf('FileCache cannot take "%s" as an option', $key));
      }*/
      $this->$key = $value;
    }
  }
  ...
  */
  protected function cleanDir($dir, $mode)
  {
    if (!($dh = opendir($dir)))
    {
      throw new CacheException('Unable to open cache directory "'.$dir.'"');
    }
  ...
  */
  protected function read($path, $file)
  {
    ...
    throw new Exception('Unable to read cache file "'.$path.$file.'"');
  }
  ...
  */
  protected function write($path, $file, $data)
  {
    ...
    throw new Exception('Unable to write cache file "'.$path.$file.'"');
  }
